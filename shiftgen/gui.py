...
class App(tk.Tk):
    ...
    def _apply_style(self):
        ...
        style.configure("Title.TLabel", background=COL_BG, foreground=COL_ACCENT_DARK, font=title_font)
        style.configure("TButton", background=COL_ACCENT, foreground="white", font=default_font, padding=(10, 6))
        style.map("TButton", background=[("active", COL_ACCENT_DARK)])

        style.configure("TLabelframe", background=COL_PANEL, bordercolor="#FED7AA", relief="solid", padding=10)
        style.configure("TLabelframe.Label", background=COL_PANEL, foreground=COL_TEXT, font=("Segoe UI", 10, "bold"))

        style.configure("Treeview", background=COL_PANEL, fieldbackground=COL_PANEL, foreground=COL_TEXT, rowheight=28, bordercolor="#FED7AA")
        style.configure("Treeview.Heading", background=COL_ACCENT_DARK, foreground="white", font=("Segoe UI", 10, "bold"), relief="flat")
    ...
    def _build_staff_panel(self, parent: ttk.Frame):
        ...
        ttk.Checkbutton(row, text="制限(平日A/土曜Bのみ)", variable=self.restricted_var).pack(side="left", padx=6)
    ...
    def _build_calendar_panel(self, parent: ttk.Frame):
        ...
        ttk.Checkbutton(top, text="祝日を自動休業にする", variable=self.auto_holiday_var, command=self._rebuild_calendar).pack(side="left", padx=8)
    ...
    def _legend_chip(self, parent: ttk.Frame, color: str, text: str):
        chip = tk.Label(parent, text=f" {text} ", bg=color, fg=COL_TEXT, font=("Segoe UI", 9, "bold"), bd=0)
    ...
    def _rebuild_calendar(self):
        ...
        self.state.auto_close_jp_holidays = bool(self.auto_holiday_var.get())
        self._jp_holidays = jp_holidays_in_month(self.state.month) if self.state.auto_close_jp_holidays else {}

        for i, w in enumerate(["日", "月", "火", "水", "木", "金", "土"]):
            ttk.Label(self.cal_frame, text=w).grid(row=0, column=i, padx=2, pady=2)

        r = 1
        c = (start.weekday() + 1) % 7  # Sunday-first
    ...
    def _load_staff_master(self):
        base = app_base_dir()
        found = find_runtime_file("staff_master.json")
        ...
        if not path.exists():
            ...
            if not picked:
                self.status_var.set(f"スタッフ読込を中止しました。({path} が見つかりません)")
                return
            path = picked

        try:
            self.status_var.set(f"スタッフ読込中: {path}")
        ...
        try:
            ...
            self.state.requests_off = {sid: ds for sid, ds in self.state.requests_off.items() if sid in valid_ids}
            self._refresh_staff_list()
            self._rebuild_calendar()
            self.status_var.set(f"スタッフを読み込みました({len(self.state.staff)}人): {path}")
        except Exception as e:
            messagebox.showerror("読込エラー", f"staff_master.json の内容が不正です。\n\n{e}")
            self.status_var.set("スタッフ読込に失敗しました。")
    ...
    def _load_template(self):
        ...
        raw = {
            ...
            "staff": [
                {"id": s.id, "name": s.name, "is_manager": s.is_manager, "allowed_kinds": list(s.allowed_kinds) if s.allowed_kinds else None}
                for s in mi.staff
            ],
            "requests_off": {sid: [d.isoformat() for d in ds] for sid, ds in mi.requests_off.items()},
        }
        self._load_from_raw(raw)
    ...
    def _load_from_raw(self, raw: dict):
        ...
        self.state.requests_off = {sid: {_parse_date(d) for d in ds} for sid, ds in raw.get("requests_off", {}).items()}
        self._refresh_staff_list()
        self._rebuild_calendar()
    ...
    def _save_json(self):
        ...
        raw = {
            ...
            "auto_close_jp_holidays": bool(self.auto_holiday_var.get()),
            "closed_dates": sorted(d.isoformat() for d in self.state.closed_dates),
            "staff": [
                {"id": s.id, "name": s.name, "is_manager": s.is_manager, "allowed_kinds": list(s.allowed_kinds) if s.allowed_kinds else None}
                for s in self.state.staff
            ],
            "requests_off": {sid: sorted(d.isoformat() for d in ds) for sid, ds in self.state.requests_off.items()},
        }
        ...
        for idx, a in enumerate(res.assignments):
            ...
            self.preview.insert(
                ...
                values=(
                    ...
                    _weekday_jp(d),
                    kind,
                    *[(staff_by_id[sid].name if (sid := a.slots.get(slot_name)) else "") for slot_name in SLOT_ORDER],
                ),
                tags=tuple(tags),
            )
